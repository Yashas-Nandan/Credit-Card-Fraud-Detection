...
# Separate class 0 (non-fraud) and class 1 (fraud)
class_0 = data[data['Class'] == 0]
class_1 = data[data['Class'] == 1]

# Split class 1 (fraud) into 70% for training and 30% for testing
class_1_train, class_1_test = train_test_split(class_1, test_size=0.3, random_state=42)

# Split class 0 (non-fraud) similarly
class_0_train, class_0_test = train_test_split(class_0, test_size=0.3, random_state=42)

# Combine the train and test sets from class 0 and class 1
train_data = pd.concat([class_0_train, class_1_train])
test_data = pd.concat([class_0_test, class_1_test])

# Shuffle the data to mix fraud and non-fraud transactions
train_data = train_data.sample(frac=1, random_state=42).reset_index(drop=True)
test_data = test_data.sample(frac=1, random_state=42).reset_index(drop=True)

# Separate features and target for the new train and test datasets
