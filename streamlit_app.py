...
def build_model(dropout_rate=0.5, neurons=128):
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(neurons, activation='relu', input_shape=(X_train_resampled.shape[1],)),
...
if uploaded_file is not None:
    try:
        ...
        X_train_class_0, X_test_class_0, y_train_class_0, y_test_class_0 = train_test_split(
            X_class_0, y_class_0, test_size=len(X_test_class_1), random_state=42, stratify=y_class_0
        )
        ...
        # Adversarial Attacks Section
            ...
            st.write("Model accuracy before attack: ", round(clean_accuracy, 2))
            st.write("Model accuracy after attack: ", round(adv_accuracy, 2))
        ...
        # Explainability Section
            ...
            feature_importance = pd.DataFrame({
                ...
                'Importance': [np.corrcoef(X[:, i], y)[0, 1] for i in range(X.shape[1])]
            })
        ...
        # Interactive Prediction Tool Section
            ...
            st.write("Input new transaction data to predict if it's fraudulent.")
            ...
            if st.button("Predict"):
                input_data_scaled = scaler.transform(np.array(input_data).reshape(1, -1))  # Scale the input
